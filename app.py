...
# Center-aligned text label using st.markdown and CSS
st.markdown(
    """
...
selected_option = st.sidebar.selectbox("Select an option", ("Single Image", "Multiple Images"))

# Display content based on the selected option
if selected_option == "Single Image":
    st.header("Select an Image")
    # Code for single image option goes here
    ...
    if st.button('Process Image'):
        if image is not None:
            ...
            if ALGO == "hier":
                ...
                if PLOTS_ON:
                    fig,ax=plt.subplots(nrows=1,ncols=1,figsize=(0.5,0.5))
                    ax.imshow(final_img)
                    ax.set_title('Original', fontsize=3)
                    plt.axis('off')
                    st.pyplot(fig)
            ...
            if PLOTS_ON:
                ...
                st.pyplot(fig)
            st.text("Loss Value:{}".format(np.mean(layer_out)))
        else:
            pass


elif selected_option == "Multiple Images":
    st.header("Select Multiple Images")
    #Code for multiple images option goes here
    ...
    if st.button('Process Images'):
        for image in images:
            st.write(f"Processing {image.name}...")
            img=Image.open(image)
            final_img=np.array(img)
            width =  512
            height =  512
            ...
            if PLOTS_ON:
                ...
                st.pyplot(fig)
            st.text("Loss Value:{}".format(np.mean(layer_out)))
            st.success(f"Processed {image.name} and saved the results.")
        else:
            pass
